...
router.post('/', async (req, res) => {
    try {
        ...
        // Check if key is completely missing or empty
        if (!process.env.GEMINI_API_KEY || process.env.GEMINI_API_KEY.trim() === '') {
            return res.status(200).json({ reply: "I'm currently running in offline mode. Please add a valid GEMINI_API_KEY to the backend .env file to enable AI responses!" });
        }
